...
### Status: MVP COMPLETE + BYOK — live at https://clauseguard.fastapicloud.dev/
...
5. **Tests**: 59/59 passing; ruff clean; mypy clean (34 files).
6. **API hardening**: 11 HTTP endpoint tests (`tests/test_routes.py`) via in-loop httpx/ASGITransport covering auth (401), 404, 413, 415, and grounded-findings happy paths. **Per-client-IP rate limiting** (sliding 60s window, in-memory, stdlib-only) enforced on all `/v1/*` routes via `Depends(rate_limit_client)`; `CLAUSEGUARD_RATE_LIMIT_PER_MINUTE` (0 = off) in `.env.example`. # ponytail: single-process limiter; move to Redis if multi-worker.
...
12. **Bring-your-own-key (BYOK) + model select**: callers post `provider` + `api_key` + `model` to run a review on their own key/model/quota, skipping the server `X-API-Key` and the daily budget (their quota, not ours). The three agents are module-scope singletons; the job swaps them via pydantic-ai `Agent.override(model=...)` inside an `ExitStack` for the duration of `review_contract()` — the *same* contextvar mechanism the tests use with `TestModel`, so retries/batching/guardrails are untouched. `GET /v1/models` (open) serves the Groq/OpenRouter catalog; `GET /v1/playbooks` and `GET /v1/reviews/{id}` are now open too, with the limiter re-bucketed from per-key to per-client-IP (reachable endpoints carry no key; BYOK spends the caller's quota). POST review endpoints still require BYOK trio or a valid `X-API-Key` (401), half-filled BYOK is 400. `build_model()` renders provider from a `Settings.model_copy()` per request — zero new factory surface. Frontend: ConfigCard (provider/model/key, persisted in localStorage) gates the Run buttons; `VITE_API_BASE_URL` points a separately-hosted UI at the backend. 59/59 tests, ruff+mypy clean.
