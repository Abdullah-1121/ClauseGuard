...
function Strip({ result, deviations, onReset, count }) {
  return (
    <div className="flex items-center gap-5 border-b border-hairline bg-ink-900 px-6 py-3">
      ...
      <Metric label="Deviations" value={deviations} accent=" text-red-400" />
      ...
      <div className="ml-auto flex items-center gap-4">
        ...
        <button
          onClick={onReset}
          className="rounded-[4px] border border-white/12 px-3 py-1.5 text-xs text-stone-400 transition-colors hover:border-white/25 hover:text-white"
        >
          New audit
        </button>
      </div>
...
function Metric({ label, value, accent = "" }) {
  return (
    <div className="flex items-baseline gap-2">
      <span className={`text-xl font-semibold tabular-nums text-white` + accent}>
        {value}
      </span>
...
function EmptyDoc({ note }) {
  return (
    <div className="flex items-center justify-center bg-ink-900/40 p-8">
      <p className="font-mono text-xs text-stone-600">{note}</p>
    </div>
  );
}
